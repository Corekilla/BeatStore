...
export async function POST(req: NextRequest) {
  ...
  if (event.type === 'checkout.session.completed') {
    ...
    const itemsWithDownloads = await Promise.all(
      rawItems.map(async (item: { beatId: string; beatTitle: string; licenseType: string; price: number }) => {
        // Each beat file is stored at: beats/{beatId}/{licenseType}.wav (or .zip for trackout)
        const filePath = `${item.beatId}/${item.licenseType}.${item.licenseType === 'trackout' ? 'zip' : 'wav'}`
