      ...
      <div className="flex flex-col gap-2.5 flex-1 min-w-0">
        ...
        {beats.map((beat, i) => {
          ...
          return (
            ...
            >
              <span aria-hidden className="absolute right-3 top-1/2 -translate-y-1/2 font-black text-6xl leading-none select-none pointer-events-none opacity-[0.035] text-white">
                {i + 1}
              </span>
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-1.5 mb-0.5">
                  ...
                  {i === 0 && (
                    <span className={`shrink-0 text-[9px] font-mono uppercase tracking-wider px-1.5 py-px rounded-sm ${rank.badgeBg} ${rank.badgeBorder} border ${rank.color}`}>
                      #1
                    </span>
                  )}
                </div>
                ...
                <div className="h-[2px] w-full bg-[var(--border)] rounded-full overflow-hidden">
                  <div className={`h-full rounded-full transition-all duration-700 ${rank.barColor}`} style={{ width: `${popularityPct}%` }} />
                </div>
              </div>

              <div className="shrink-0 text-right flex flex-col items-end gap-1.5 min-w-[64px]">
                <p className="text-[11px] font-mono font-semibold text-[var(--text)]">{formatNumber(displayCount)}</p>
                <p className={`text-[9px] font-mono uppercase tracking-wide ${rank.color}`}>{rank.label}</p>
                <button
